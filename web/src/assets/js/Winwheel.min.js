function Winwheel(a, c) {
	...
	this.animation =
		null != a && a.animation && 'undefined' !== typeof a.animation ? new Animation(a.animation) : new Animation();
	...
	if (1 == c) this.draw(this.clearTheCanvas);
		...
		for (winwheelToDrawDuringAnimation = this, winhweelAlreadyDrawn = !1, b = 1; b <= this.numSegments; b++)
...
Winwheel.prototype.draw = function (a) {
	this.ctx &&
		('undefined' !== typeof a ? 1 == a && this.clearCanvas() : this.clearCanvas(),
		'image' == this.drawMode
			? (this.drawWheelImage(),
			...
			for (var b = 1; b <= this.numSegments; b++) {
				...
				if (d.imgData.height) {
					...
					if ('S' == h) {
						...
						'E' == h
							? ((h = a), (n = c - t / 2), (r = d.startAngle + 270 + (d.endAngle - d.startAngle) / 2))
							: 'W' == h
							? ((h = a - e), (n = c - t / 2), (r = d.startAngle + 90 + (d.endAngle - d.startAngle) / 2))
							: ((h = a - e / 2), (n = c - t), (r = d.startAngle + (d.endAngle - d.startAngle) / 2));
					this.ctx.save();
					this.ctx.translate(a, c);
					...
					if ('reversed' == t)
						if ('horizontal' == d)
							(this.ctx.textAlign = 'inner' == e ? 'right' : 'outer' == e ? 'left' : 'center'),
								...
								this.ctx.translate(-l, -p),
								'inner' == e
									? (n && this.ctx.fillText(a[f], l - v - h, p + b), r && this.ctx.strokeText(a[f], l - v - h, p + b))
									: 'outer' == e
									? (n && this.ctx.fillText(a[f], l - w + h, p + b), r && this.ctx.strokeText(a[f], l - w + h, p + b))
								...
								for (
									m = 0,
										'inner' == e
											...
											: 'outer' == e
											? ((m = w - h), (this.ctx.textBaseline = 'bottom'), (m -= c * (a.length - 1)))
											: 'center' == e && ((m = v + h + (w - v) / 2), (this.ctx.textBaseline = 'middle')),
										k = 0,
										1 < a[f].length
											? ((this.ctx.textAlign = 'left'),
											  (k = (c / 10) * 4),
											  (k *= 100 / m),
											  (q = g.startAngle + ((g.endAngle - g.startAngle) / 2 - (k * a[f].length) / 2)))
											: ((q = g.startAngle + (g.endAngle - g.startAngle) / 2), (this.ctx.textAlign = 'center')),
						...
						if ('outer' == e)
							...
							for (
								k = 0, 1 < a[f].length && (k = (m * (a[f].length - 1)) / 2), k = p - v - (w - v) / 2 - k - h, q = 0;
								q < a[f].length;
								q++
							)
								(u = a[f].charAt(q)),
						...
						for (
							m = 0,
								'inner' == e
									? ((m = v + h), (this.ctx.textBaseline = 'bottom'), (m += c * (a.length - 1)))
									: 'outer' == e
...
Winwheel.prototype.stopAnimation = function (a) {
	winwheelToDrawDuringAnimation &&
		(winwheelToDrawDuringAnimation.tween && winwheelToDrawDuringAnimation.tween.kill(), winwheelStopAnimation(a));
	winwheelToDrawDuringAnimation = this;
};
...
function winwheelTriggerSound() {
	...
	c != winwheelToDrawDuringAnimation._lastSoundTriggerNumber &&
		('function' === typeof a ? a() : eval(a), (winwheelToDrawDuringAnimation._lastSoundTriggerNumber = c));
}
