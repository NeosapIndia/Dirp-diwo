...
@Component({
	selector: 'app-manage-assets-library',
	templateUrl: './manage-assets-library.component.html',
	styleUrls: ['./manage-assets-library.component.scss'],
	animations: [routerTransition()],
	providers: [ConfirmationService],
})
export class ManageAssetsLibraryComponent implements OnInit, AfterViewChecked {
	...
	constructor(
		...
		private manageAssetLibService: ManageAssetsLibraryService,
	...
	ngOnInit() {
		...
		if (JSON.parse(localStorage.getItem('assetPageNo'))) {
			let pageDetails = JSON.parse(localStorage.getItem('assetPageNo'));
		...
		localStorage.removeItem('assetPageNo');
		this.getAppBranding();
	}
	...
	getAllAssetByBuyer(clientId, page, limit) {
		...
		this.manageAssetLibService.getAllAssetsByBuyer(clientId, page, limit).subscribe((res: any) => {
			if (res.success) {
				this.allAssetList = [];
	...
	addAssets() {
		if (!this.writePermission) {
			...
			this.router.navigate(['assets-library/add-or-edit-assets']);
		}
	}
	...
	editAsset(asset) {
		if (!this.writePermission) {
			...
			this.router.navigate(['assets-library/add-or-edit-assets', { assetId: assetId }]);
		}
	}
	...
	removeAsset() {
		this.spinnerService.show();
		this.manageAssetLibService
			.deleteAssetsByBuyer(this.selectedAssetIdFordelete, this.selectedClient.id)
			.subscribe((res: any) => {
				if (res.success) {
					$('#deleteAssetModal').modal('hide');
					this.selectedAssetIdFordelete = null;
					...
					this.toastr.success(
						this.appService.getTranslation('Pages.Assets.Home.Toaster.assetsdeleted'),
						this.appService.getTranslation('Utils.success')
					);
	...
	getAssetsByFilter(key) {
		...
		if (this.inputSearchTxt && this.inputSearchTxt.length > chars && this.inputSearchTxt.length !== 0) {
			...
			this.typingTimer = setTimeout(() => {
				this.manageAssetLibService.getFilteredAssets(this.payload, this.page, this.limit).subscribe(
					(res: any) => {
						this.isApiCall = false;
						this.isDataLoaded = true;
						if (res.success) {
	...
	onCheck(value, item, index) {
		...
		if (this.selectedAssetsIds.indexOf(item.id) > -1) {
			this.selectedAssetsIds.splice(this.selectedAssetsIds.indexOf(item.id), 1);
