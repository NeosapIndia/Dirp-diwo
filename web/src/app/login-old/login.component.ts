...
export class LoginComponent implements OnInit, OnDestroy {
	...
	ipAddress = '';
	customerRollData = [];
	value = localStorage.getItem('instalmentPage');
	redirectUrlToCheck = '';
	...
	pendingOnboardingActions = false;
	messageBoard = false;
	messageBoardList = [];
	signInMessage: any;
	displayMessage = false;
	loginAllowedFlag = true;
	beforwMaintence: boolean = false;
	messageIsDuringFlag: boolean = false;
	fromDate: string = '';
	toDate: string = '';
	fromdate: string;
	todate: string;
	...
	fromYear: string;
	toYear: string;
	alertMessageFlag: boolean = false;
	restrictedCareGiver = false;
	...
	constructor(
		...
		public appService: AppService, // To initialize default language
	...
	ngOnInit() {
		this.loginForm = this.formBuilder.group({
			...
			// email: ['', !this.isMobileLogin ? [Validators.required, Validators.pattern(/^\w+([\.-]?\w+.*\.*)*@\w+([\.-]?\w+)*(\.\w{2,})+$/)] : null],
			email: ['', !this.isMobileLogin ? [Validators.required] : null],
		...
		if (localStorage.getItem('user')) {
			...
			if (user.user.access_level == 'Restricted' && user.user.type == 'Care Giver') this.restrictedCareGiver = true;
		}
		let count = 0;
		// let hideChat = setInterval(()=>{
		//   count++;
		//   let varebale = document.getElementById("reve-chat-container-div");
		//   if (varebale) {
		//     varebale.style.display = 'none';
	...
	changeCountry(country) {
		...
		this.loginForm = this.formBuilder.group({
			...
			email: [
				'',
				!this.isMobileLogin
					? [Validators.required, Validators.pattern(/^\w+([\.-]?\w+.*\.*)*@\w+([\.-]?\w+)*(\.\w{2,})+$/)]
					: null,
			],
		...
		this.authenticationService
			...
			.subscribe(
				(data) => {
					...
					this.loginBy = '';
					if (data.success) {
						//localStorage.setItem("login_by",Object.keys(verificationMedium)[0]);
	...
	onSubmit() {
		...
		this.spinnerService.show();
		this.loading = true;
		const otp = this.o.otp1.value + '' + this.o.otp2.value + '' + this.o.otp3.value + '' + this.o.otp4.value;
		let verificationMedium =
			this.f.phone.value.toString() == 'null' || this.f.phone.value.toString() == ''
				? { email: this.f.email.value.toString() }
				: { phone: this.f.phone.value.toString() };
	...
	validate(data) {
		...
		if (data.user && (flag || admin_roles.indexOf(data.user.roles[0].role) > -1)) {
			...
			if (this.userRoles.length > 1) {
				...
				//console.log('----------22-------', data.user.roles[0].client);
	...
	async getAppBranding(userClient, data) {
		this.appService.getAppBranding(userClient.id).subscribe((res: any) => {
			if (res.success) {
				...
				setTimeout(() => {
					//console.log('-----------Show Login By Role', this.appService.whiteBrandingColor);
					this.loginByRole(data);
				}, 300);
			}
		});
	...
	customerLogin(data, redirectUrl, isMobile) {
		if (isMobile) {
			localStorage.setItem('isLoggedin', 'true');
			// this.router.navigate(['/consumer/starthome']);
			return;
		}
		let marketId;
		...
		// if (data.user && (data.user.roles.indexOf('Customer') > -1)) {
		//   this.appService.getUserAllCustomerRoleData(data.user.id, marketId).subscribe((res: any) => {
		//     if (res.success) {
		//       if (res.data.length > 1) {
		//         this.spinnerService.hide();
		//         this.customerRollData = res.data;
		//         $('#customerLogin').modal('show');
		//       } else {
		//         const perm = ["CUSTOMER"];
		//         this.permissionsService.loadPermissions(perm);
		//         if (!(data && data.child && Object.keys(data.child).length !== 0) && (adminrole == null || adminrole == 'null' || adminrole == "")) {
		//           localStorage.removeItem('redirectUrl');
		//           this.router.navigate(['/add-baby']);
		//         } else {
		//           if (this.value == "true") {
		//             localStorage.setItem("instalmentPage", 'false');
		//             this.router.navigate(['/consumer/instalment']);
		//           } else {
		//             this.redirectUrlToCheck = redirectUrl;
		//             if (redirectUrl != null) {
		//               var url = redirectUrl.split('/#');
		//               if (url[1].includes('whybablr')) {
		...
		//               }
		//             } else {
		//               let tempPackageUrl = localStorage.getItem('tempPackageUrl');
		//               if (tempPackageUrl != null && tempPackageUrl != "null" && tempPackageUrl != "") {
		//                 if (localStorage.getItem('buyPage')) {
		//                   this.router.navigate(['/consumer/cart'], { queryParams: { id: localStorage.getItem('tempPackageId'), 'action': 'beforeLoginUser' } });
		//                   localStorage.removeItem('buyPage');
		//                   localStorage.removeItem('tempPackageId');
		//                 }  else {
		//                   if (!(data && data.child && Object.keys(data.child).length !== 0) && (adminrole == null || adminrole == 'null' || adminrole == "")) {
		//                     this.router.navigate(['/add-baby']);
		//                   } else {
		...
		//                 if (!(data && data.child && Object.keys(data.child).length !== 0) && (adminrole == null || adminrole == 'null' || adminrole == "")) {
		//                   this.router.navigate(['/add-baby']);
		//                 } else if (localStorage.getItem('giftABablrPage')) {
		//                   this.router.navigate(['/consumer/giftHome']);
		//                   localStorage.removeItem('giftABablrPage');
		//                 }else {
		//                   this.goToTodayPage();
		//                 }
		//               }
	...
	checkUserType(data) {
		...
		this.spinnerService.show();
		this.appService.updateUserRoleDataInUserTable(data).subscribe(
			(res: any) => {
				if (res.success) {
					let user = JSON.parse(localStorage.getItem('user'));
					let user_ = user;
					user_.token = res.token;
					user_.user = res.user;
					user_.user = this.appService.removePersonalData(user_.user);
					localStorage.setItem('user', JSON.stringify(user_));
					this.appService.setUserPersonalData();
					// console.log('------------------1-----------------');
					this.appService.checkGoogleDriveDetails();
					if (data.type == 'Care Giver') {
						...
						this.permissionsService.loadPermissions(perm);
						let adminrole = localStorage.getItem('adminrole');
						if (res && res.childCount == 0 && (adminrole == null || adminrole == 'null' || adminrole == '')) {
							localStorage.removeItem('redirectUrl');
							this.router.navigate(['/add-baby']);
						} else {
							if (this.value == 'true') {
								...
								if (this.redirectUrlToCheck != null && this.redirectUrlToCheck != '') {
									var url = this.redirectUrlToCheck.split('/#');
									if (url[1] && url[1].includes('whybablr')) {
										this.goToTodayPage();
									} else if (url[1]) {
										this.router.navigate([url[1]]);
									}
									...
									if (tempPackageUrl != null && tempPackageUrl != 'null' && tempPackageUrl != '') {
										if (localStorage.getItem('buyPage')) {
											this.router.navigate(['/consumer/cart'], {
												queryParams: {
													id: localStorage.getItem('tempPackageId'),
													action: 'beforeLoginUser',
												},
											});
											localStorage.removeItem('buyPage');
											localStorage.removeItem('tempPackageId');
	...
	goToTodayPage() {
		...
		let adminrole = localStorage.getItem('adminrole');
		if (adminrole == null || adminrole == 'null' || adminrole == '') {
			this.pendingOnboardingActions = false;
			let user = JSON.parse(localStorage.getItem('user'));
			let childData = user.child;
			let userData = user.user;

			this.appService.getUserCookiePreferences(userData.id).subscribe(async (res: any) => {
				if (res.success === true) {
					if (res.data && res.data.other_details) {
						localStorage.setItem('Cookie Preferences', res.data.other_details);
					}
				}
			});
			this.appService.getPendingActionList(userData.id).subscribe(
				async (res: any) => {
					if (res.success === true) {
						if (res.data && res.data.details) {
							let pendingActionData = JSON.parse(res.data.details);
							if (childData && childData.child_name_for_certificate) {
								for (let i in pendingActionData) {
									if (pendingActionData[i].name == 'Certificate Name') {
										pendingActionData[i].dontShowAgain = true;
									}
								}
							}
							let object = {
								details: JSON.stringify(pendingActionData),
								userId: userData.id,
							};
							this.appService.updatePendingActionList(object).subscribe(async (res: any) => {});

							for (let i in pendingActionData) {
								if (!pendingActionData[i].dontShowAgain) {
									this.pendingOnboardingActions = true;
									this.messageBoard = true;
								}
							}
						} else {
							let pendingActionList = [
								{ name: 'How To', dontShowAgain: false },
								{
									name: 'Child laguages',
									dontShowAgain: false,
								},
								{ name: 'Rotuine', dontShowAgain: false },
								{
							...
							if (childData && childData.child_name_for_certificate) {
								for (let i in pendingActionList) {
									if (pendingActionList[i].name == 'Certificate Name') {
										pendingActionList[i].dontShowAgain = true;
										this.messageBoard = true;
									}
								}
							}
							let object = {
								details: JSON.stringify(pendingActionList),
								UserId: userData.id,
								MarketId: userData.MarketId,
							};
							this.appService.addPendingActionList(object).subscribe(async (res: any) => {});

							for (let i in pendingActionList) {
								if (!pendingActionList[i].dontShowAgain) {
									this.pendingOnboardingActions = true;
									this.messageBoard = true;
								}
							}
						...
						this.toastr.error(this.appService.getTranslation('Utils.somthingwentwrong'));
					}
				},
				(error) => {}
			);
	...
	getMessageBoardData() {
		...
		let childData = user.child;
		let userData = user.user;
		let Market = userData.Market;
		let childId = childData && childData.id ? childData.id : null;

		this.appService.getMessagesForUser(userData.id, childId, Market.name).subscribe(
			async (res: any) => {
				if (res.success === true) {
					this.messageBoardList = res.data;
					if (!this.messageBoard) {
						if (this.messageBoardList.length > 0) {
							this.messageBoard = true;
						}
					}
					...
					if (!this.messageBoard) {
						...
						this.spinnerService.hide();
						//open message board popup
						if (localStorage.getItem('user')) {
							let user = JSON.parse(localStorage.getItem('user'));
							if (user.user.access_level == 'Restricted' && user.user.type == 'Care Giver')
								this.restrictedCareGiver = true;
						}
						$('#massageBoard').modal('show');
					}
				} else {
					this.toastr.error(this.appService.getTranslation('Utils.somthingwentwrong'));
				}
