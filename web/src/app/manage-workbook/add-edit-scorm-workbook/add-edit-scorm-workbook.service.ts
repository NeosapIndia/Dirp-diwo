...
export class ManageAddEditScormWorkbookService {
	...
	uploadExtractZipDiwoAsset(
		data: FormData,
		trackProgress: boolean = false
		): Observable<any | HttpEvent<any>> {
		if (trackProgress) {
			return this.http.post<any>(`${this.apiHostUrl}/upload-extract-zip-diwo-asset`, data, {
			reportProgress: true,
			observe: 'events'		
			});
		} else {
	...
	uploadZipDiwoAssetBlob(data: any): Observable<ArrayBuffer> {
		return this.http.post(
			`${this.apiHostUrl}/scorm/get-zip-content`,
			data,
	...
	createScormWorkbook(data, clientId) {
		return this.http.post(`${this.apiHostUrl}/${clientId}/create-scorm-workbook`, data).pipe(
			map((data) => {
				return data;
			})
		);
	...
	updateScormWorkbook(data, clientId, workbookId) {
		return this.http.put(`${this.apiHostUrl}/${clientId}/update-scorm-workbook/${workbookId}`, data).pipe(
			map((data) => {
				return data;
			})
		);
	...
	uploadVideoOnMediaCMS(data: FormData, clientId: number): Observable<HttpEvent<any>> {
		return this.http.post<any>(`${this.apiHostUrl}/${clientId}/upload-video-on-mediacms`, data, {
			reportProgress: true,
			observe: 'events',
		});
	}
	...
	checkMediaCMSVideoTranscodingStatus(payload) {
		return this.http.post(`${this.apiHostUrl}/check-mediaCMS-transcoding-status`, payload).pipe(
			map((data) => {
				return data;
			})
		);
	...
	bulkUploadWorksheet(data) {
		return this.http.post(`${this.apiHostUrl}/create-worksheet-in-bulk`, data).pipe(
			map((data) => {
				return data;
			})
		);
