...
@Component({
	selector: 'app-see-all-todos',
	templateUrl: './see-all-todos.page.html',
	styleUrls: ['./see-all-todos.page.scss'],
})
export class SeeAllTodosPage implements OnInit {
	...
	filteredAssignToDosListData: any = [];
	searchQuery: string = '';	
	...
	getUserAssignToDosList() {
		this.appService.getUserAssignSeeAllToDosList().subscribe((res: any) => {
			if (res.success) {
				if (res.data && res.data.toDosList) {
					...
					this.filteredAssignToDosListData = [...this.assignToDosListData];

					// Restore previous search filter if it exists
					const savedQuery = localStorage.getItem('searchTodoListQuery');
					if (savedQuery) {
	...
	filterData() {
		...
		localStorage.setItem('searchTodoListQuery', this.searchQuery);
		...
		this.filteredAssignToDosListData = this.assignToDosListData.filter(
			(item) =>
