...
export class ManageAssetsLibraryService {
	...
	getAllAssetsByBuyer(clientId, page, limit) {
		return this.http.get(`${this.apiHostUrl}/${clientId}/get-all-asset-by-client?page=${page}&limit=${limit}`).pipe(
			map((data) => {
				return data;
			})
		);
	...
	deleteAssetsByBuyer(assetId, clientId) {
		return this.http.put(`${this.apiHostUrl}/${clientId}/delete-asset`, assetId).pipe(
			map((data) => {
				return data;
			})
		);
	...
	getFilteredAssets(payload, page, limit) {
		return this.http.post(`${this.apiHostUrl}/search/assets?page=${page}&limit=${limit}`, payload).pipe(
			map((data) => {
				return data;
			})
		);
