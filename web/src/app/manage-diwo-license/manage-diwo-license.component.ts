...
@Component({
	selector: 'app-manage-diwo-license',
	templateUrl: './manage-diwo-license.component.html',
	styleUrls: ['./manage-diwo-license.component.scss'],
	animations: [routerTransition()],
})
export class ManageDiwoLicenseComponent implements OnInit, AfterViewChecked {
	ClientData: any[] = [];
	...
	FilterColumnArray = [
		...
		{ label: 'Learner Count', value: 'learnerCount' },
		{ label: 'Diwo Volume', value: 'diwoVolume' },
	...
	ngOnInit() {
		...
		let searchKeyData: any = JSON.parse(localStorage.getItem('searchDiwoLicenses'));
		if (searchKeyData) {
	...
	addLicense() {
		if (!this.writePermission) {
			...
			this.router.navigate(['/diwo-licenses/add-or-edit-diwo-license']);
		}
	}
	...
	editClient(editClientData) {
		if (!this.writePermission) {
			...
			this.router.navigate(['/diwo-licenses/add-or-edit-diwo-license'], {
				queryParams: { clientId: editClientData.id },
			});
		}
	}

	viewClient(editClientData) {
		this.router.navigate(['/diwo-licenses/view-diwo-license'], { queryParams: { clientId: editClientData.ClientId } });
	}
	...
	onSuspendFormSubmit() {
		...
		this.LicenseService.suspendClient(this.selectedClientForSuspend).subscribe((res: any) => {
			if (res.success) {
				...
				this.toastr.success(
					this.appService.getTranslation('Pages.DiwoLicense.Home.Toaster.suspendlicense'),
		...
		this.LicenseService.activateSuspendClient(this.selectedClientForActivate).subscribe((res: any) => {
			if (res.success) {
				...
				if (res.message == 'License expired!') {
					this.toastr.success(
						this.appService.getTranslation('Pages.DiwoLicense.Home.Toaster.expirelicense'),
						this.appService.getTranslation('Utils.success')
					);
				} else {
					this.toastr.success(
						this.appService.getTranslation('Pages.DiwoLicense.Home.Toaster.activatelicense'),
						this.appService.getTranslation('Utils.success')
					);
				}
				this.spinnerService.hide();
	...
	getLicenseByFilter(key) {
		...
		localStorage.setItem('searchDiwoLicenses', JSON.stringify(this.payload));
